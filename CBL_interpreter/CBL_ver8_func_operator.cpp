...
#define IF 19 // IF 조건문
#define ELIF 20 // elseif 조건문
#define ELSE 21 // ELSE 조건문
...
tokens assign_token; // 대입부 연산시 임시저장 토큰
bool sub1[8];
bool sub2[8];
bool sub_result[8];
//변수를 저장하는 클래스
...
int lex() {
	...
	case LETTER:
		...
		if (store == "def") // 함수라면
			...
			return PRINT;
		else if (store == "if") // if 발견시
			return IF;
		else if (store == "elif") // elif 발견시
			return ELIF;
		else if (store == "else") // else 발견시
			return ELSE;
		else // 그외 문자라면
...
bool ifelse() {
	addfunctoken(); // IF 추가
	nexttoken(); // 다음 토큰
	if (sub_tokens.type == IDENT || sub_tokens.type == BINARY) { // 다음 토큰이 변수 또는 2진상수라면
		addfunctoken();
		nexttoken(); // 다음 토큰
		if (sub_tokens.type == BOOLO || sub_tokens.type == BOOLN ||
			sub_tokens.type == EQUALS || sub_tokens.type == NEQUALS) { // 다음 토큰이 논리 연산자라면
			addfunctoken();
			nexttoken(); // 다음 토큰
			if (sub_tokens.type == IDENT || sub_tokens.type == BINARY) { // 다음 토큰이 변수 또는 2진상수라면
				addfunctoken();
				nexttoken(); // 다음 토큰
				if (sub_tokens.type == COLON) { // 다음 토큰이 콜론이 맞다면
					addfunctoken();
					nexttoken(); // 다음 토큰
					if (sub_tokens.type == LF) { // 다음 토큰이 개행문자라면
						addfunctoken();
						nexttoken(); // 다음 토큰
						while (sub_tokens.type != IDENT) {
							if (sub_tokens.type == HT) {
								addfunctoken();
								nexttoken(); // 다음 토큰
								if (sub_tokens.type == HT) { // 탭이 두개인지 확인.
									addfunctoken();
									nexttoken(); // 다음 토큰
									if (!stmt()) // stmt 호출
										return false;
									if (sub_tokens.type == LF) {
										addfunctoken();
										nexttoken(); // 다음 토큰
									}
									else false;
								}
								else if (sub_tokens.type == ELIF)
									break;
								else if (sub_tokens.type == ELSE)
									break;
								else if (sub_tokens.type == IDENT || sub_tokens.type == VAR || sub_tokens.type == PRINT) {
									defvec.at(func_size).tovec.pop_back(); // 탭 제거.
									token_pos--; // 토큰 -1
									token_pos--; // 토큰 -2 -> nexttoken 시 \t불러와짐
									break;
								}
								else false;
							}
							else false;
							if (lexer.size() == token_pos) break;
						}
						if (sub_tokens.type == ELIF) { //ELIF 발견시
							addfunctoken(); // ELIF 추가
							nexttoken(); // 다음 토큰
							if (sub_tokens.type == IDENT || sub_tokens.type == BINARY) { // 다음 토큰이 변수 또는 2진상수라면
								addfunctoken();
								nexttoken(); // 다음 토큰
...
void interpreter() {
	...
	while (main_size != inter_pos) {
		...
		if (inter_token.type == VAR) // VAR를 발견하엿을 경우.
			...
			var_print();
		else if (inter_token.type == IF) // 조건문 발견
			var_ifelse();
	}
	cout << endl << endl;
...
void var_assign() {
	...
	if (inter_token.type == BINARY) { // 다음 토큰이 2진상수일 경우.
		assign_token = inter_token;
		inter_next();
		if (inter_token.type == AND||
			inter_token.type == OR||
			inter_token.type == XOR) {
			//sub1에 첫번째 값 저장.
			if (assign_token.token.at(9) == '1')
				sub1[0] = true;
			else
				sub1[0] = false;
			if (assign_token.token.at(8) == '1')
				sub1[1] = true;
			else
				sub1[1] = false;
			if (assign_token.token.at(7) == '1')
				sub1[2] = true;
			else
				sub1[2] = false;
			if (assign_token.token.at(6) == '1')
				sub1[3] = true;
			else
				sub1[3] = false;
			if (assign_token.token.at(5) == '1')
				sub1[4] = true;
			else
				sub1[4] = false;
			if (assign_token.token.at(4) == '1')
				sub1[5] = true;
			else
				sub1[5] = false;
			if (assign_token.token.at(3) == '1')
				sub1[6] = true;
			else
				sub1[6] = false;
			if (assign_token.token.at(2) == '1')
				sub1[7] = true;
			else
				sub1[7] = false;
			int op_type = inter_token.type; // 연산자 타입 저장
			inter_next();
			//sub2 값 대입
			if (inter_token.type == BINARY) {
			...
			else if (inter_token.type == IDENT) {
				int ident_pos = 0;
				...
				if (memory.at(depth).at(ident_pos).binary[0])
					sub2[0]= true;
				else
					sub2[0] = false;
				if (memory.at(depth).at(ident_pos).binary[1])
					sub2[1] = true;
				else
					sub2[1] = false;
				if (memory.at(depth).at(ident_pos).binary[2])
					sub2[2] = true;
				else
					sub2[2] = false;
				if (memory.at(depth).at(ident_pos).binary[3])
					sub2[3] = true;
				else
					sub2[3] = false;
				if (memory.at(depth).at(ident_pos).binary[4])
					sub2[4] = true;
				else
					sub2[4] = false;
				if (memory.at(depth).at(ident_pos).binary[5])
					sub2[5] = true;
				else
					sub2[5] = false;
				if (memory.at(depth).at(ident_pos).binary[6])
					sub2[6] = true;
				else
					sub2[6] = false;
				if (memory.at(depth).at(ident_pos).binary[7])
					sub2[7] = true;
				else
					sub2[7] = false;
			}
...
void var_ifelse() {
	inter_next(); // ident
	ident_token = inter_token;
	inter_next();
	int sub_type = inter_token.type;
	inter_next();
	int a_pos = 0;
	int b_pos = 0;
	for (var v : memory.at(depth)) {
	...
	if (sub_type == EQUALS) {
		checking = true;
		for (int i = 0; i < 8; i++) {
			if (memory.at(depth).at(a_pos).binary[i]
				!= memory.at(depth).at(b_pos).binary[i]) {
				checking = false;
				break;
			}//다른게 있다면
		}
...
void func_assign() {
	...
	if (func_token.type == BINARY) { // 다음 토큰이 2진상수일 경우.
		assign_token = func_token;
		func_next();
		if (func_token.type == AND ||
			func_token.type == OR ||
			func_token.type == XOR) {
			//sub1에 첫번째 값 저장.
			if (assign_token.token.at(9) == '1')
			...
			if (assign_token.token.at(2) == '1')
				...
				sub1[7] = false;
			int op_type = func_token.type; // 연산자 타입 저장
			func_next();
			//sub2 값 대입
			if (func_token.type == BINARY) {
			...
			callfvec.at(callfvec.size() - 1).call_pos--;
			callfvec.at(callfvec.size() - 1).call_pos--;
			func_next();
			int ident_pos = 0;
			for (var v : memory.at(depth)) {
...
void assign_xor() {
	for (int i = 0; i < 8; i++) {
		if ((sub1[i] & !sub2[i]) || (!sub1[i] & sub2[i]))
			sub_result[i] = true;
		else
			sub_result[i] = false;
	}
